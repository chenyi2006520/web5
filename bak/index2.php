...
#echo $maxkey.'<br>';
#好搜手机版关键词新闻搜索返回页
#$haosou='http://m.news.haosou.com/ns?q=%E8%B6%B3%E7%90%83';
#$haosou='http://m.news.haosou.com/ns?q=%E6%9C%BA%E7%A5%A8';
$num=0;
$lock= mysql_fetch_row(mysql_query("SELECT lswitch FROM locker where lid = 1"));
$lswitch=$lock[0];
#echo 'suo'.$lswitch;
$suo=0;
if ($lswitch==0 && $maxakeyid==0) {
    #echo "抓了";
mysql_query("UPDATE locker SET lswitch = 1 WHERE lid=1 ");
$suo=1;
#抓取所有内容至$news
$news=file_get_contents($haosou);
#匹配文章列表部分
preg_match('/<div id=main>(.*?)相关搜索/si',$news,$m);
#匹配所有文章URL
preg_match_all('/<a href="(.*?)">/si',$m[1],$url);
#匹配所有文章标题
preg_match_all('/<h3 class=title>(.*?)<\/h3>/si',$m[1],$raw_title);
#echo strip_tags($raw_title[1][0]).'<br>';
#for ($key_num=0; $key_num<count($url[1]) ; $key_num++) { 
for ($key_num=0; $key_num<5 ; $key_num++) { 
    if ($maxtitle!=strip_tags($raw_title[1][$key_num])) {
    }
    else {
    break;
    }
...
for ($i=0; $i<$key_num ; $i++) { 
    ...
    if (isset($title_raw[1])) {
        ...
        preg_match('/<div id="news-body">(.*?)<\/div>/si',$article_raw,$content_raw);
        #$link[$num]= $link_raw;
...
if (empty($kname)) {
    ...
    #echo '循环次数'.$i;
#mysql_query("UPDATE keyword SET kamont = kamont+'$num' WHERE kid='$kid'");
?>
<!DOCTYPE html>
